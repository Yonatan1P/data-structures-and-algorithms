...
Build a simple express server. Connect a '/hello' route that sends a greeting of your
choice. Connect a '/aboutme' route that sends a short bio about you to the front-end.
Finally, connect a '/favoritefoods' route that sends an array to the front-end of your
 favorite foods. All other routes should respond with a status of 404.
------------------------------------------------------------------------------------------------ */
...
const createServer = () => {
 ...
 app.get('/favoritefoods', handleFavoriteFoods);
 app.get('*', handleError);
 function handleHello(request, response) {
    response.send('Hi!');
 }
 function handleAboutMe(request, response){
     response.send('I am a Russian Jew who studied biophysics.')
 }
 function handleFavoriteFoods(request, response){
    let favoriteFoods = ['pizza','hotdogs','chocolate'];
    response.send(favoriteFoods);
 }
...
filter, map, or reduce to count the amount of times the integer is present in the array
of arrays.
