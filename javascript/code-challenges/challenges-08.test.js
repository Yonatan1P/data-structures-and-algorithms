...
Write a function named sayHello, that sends the message 'Hello from the back-end'
 when a user hits the `/hello` route.
...
function sayHello(request, response){
  let respond =response.send('Hello from the back-end');// Solution code here...
    return respond;
}
...
const oddValues = (arr) => {
    ...
    return odd; // Solution code here...
};

/* ------------------------------------------------------------------------------------------------
CHALLENGE 3

Write a function named filterStringsWithVowels that, given an array of strings
 as input, uses filter to return an array with only words that contain vowels.

The callback function to filter should include or utilize a regular expression
 pattern.

For example, filterStringsWithVowels('gregor','hound','xyz') 
             returns ['gregor', 'hound'].
------------------------------------------------------------------------------------------------ */
...
const notInFirstArray = (forbiddenValues, arr) => {
  const newArr = [];
    arr.forEach(goodNum =>{
     if (!forbiddenValues.includes(goodNum)){
        newArr.push(goodNum);
     }
    });
