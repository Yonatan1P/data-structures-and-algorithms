...
const firstLetters = (arr) => {
 ...
 let ans = [];
 arr.forEach(string =>{
    let firstLetter = string.match(regex); 
    ans.push(firstLetter[0]);
 }) 
 return ans;
};
...
Write a function named standardizePhoneNumbers that takes in an array of phone number
 strings in (XXX) XXX-XXXX format and returns an array with the phone number strings 
 in XXXXXXXXXX format.
...
Write a function named onlyOddChars that takes in a string and returns only the 
odd-index characters from that string.
